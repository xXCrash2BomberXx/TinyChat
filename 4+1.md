...
```mermaid
graph LR
  A[User Interface] --> |Establish Connection| B[Web Browser];
  B --> |Update User Interface| A
  B --> |Send Message| C[Database];
  C --> |Receive Message| B;
```
...
stateDiagram
  ...
  state Communication {
    ...
    StopTyping --> StopTypingIndicatorSent
    StopTypingIndicatorSent --> StopTypingIndicatorReceived
    SendMessage --> RenderMessage
    DecryptMessage --> RenderMessage
    ReceivedDeliveryReceipt --> RenderMessage
    UnsendMessage --> RenderMessage
    StopTypingIndicatorReceived --> RenderMessage
  }
